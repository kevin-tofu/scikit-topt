...
def test_hamilton_jacobi_WENO_like(basis_s, φ, V):
...
def get_dVol_assembler() -> Callable:
    ...
    def dVol_assembler(v, w):
        ...
        q_val = w.q_interp                            # (dim, nqp, nelems)
...
def node_to_element_density_averaging(mesh: skfem.Mesh, rho_node: np.ndarray) -> np.ndarray:
    rho_elem = np.zeros(mesh.t.shape[1])
    for e, nodes in enumerate(mesh.t.T):
        rho_elem[e] = np.mean(rho_node[nodes])
    return rho_elem


def assemble_stiffness_matrix_with_φ(basis_v, basis_s, φ, Emax, Emin, nu):
    E_node = Emin + (Emax - Emin) * heaviside(φ)
    lam_node = (nu * E_node) / ((1.0 + nu) * (1.0 - 2.0 * nu))
    mu_node = E_node / (2.0 * (1.0 + nu))

    lam_interp = basis_s.interpolate(lam_node)
    mu_interp  = basis_s.interpolate(mu_node)

    @skfem.BilinearForm
    def stiffness_form(u, v, w):
        eps_u = sym_grad(u)
        eps_v = sym_grad(v)
        # trace(eps_u), trace(eps_v) は (n_qpoints, n_elems) になる
        return lam_interp * trace(eps_u) * trace(eps_v) + \
               2.0 * mu_interp * ddot(eps_u, eps_v)
...
def test_fem(
    ...
    print("φ range:", np.min(φ), np.max(φ))
...
def compute_objectives(
    ...
    L = skfem.asm(lform, basis_s, Hφ=heaviside(φ))
    total = skfem.asm(skfem.LinearForm(lambda v, w: np.ones_like(v)), basis_s)
    Vol = np.sum(L) / np.sum(total)
    return J, Vol


def compute_derivatices(E, nu, basis_v, basis_s, u, q, φ):
    ...
    # dJ_norm = np.max(np.abs(dJ))
    # dVol_norm = np.max(np.abs(dVol))
    # print(f"dJ_norm={dJ_norm:.3e}, dVol_norm={dVol_norm:.3e}")

    return dJ, dVol, φ_grad_norm


def hamilton_jacobi_RK4_rhs(
    Emax, nu, basis_v, basis_s, λ, u, q, φ
):

    dJ, dVol, φ_grad_norm = compute_derivatices(
        Emax, nu, basis_v, basis_s, u, q, φ
    )
    # velocity = - (dJ + λ * dVol)
    velocity = - (dJ - λ * dVol)
    maxv = np.max(np.abs(velocity)) + 1e-16
    velocity /= maxv
    return velocity


def hamilton_jacobi_RK4(
    Emax, nu, basis_v, basis_s, λ, u, q,
    φ,
    dt
):
    k1 = hamilton_jacobi_RK4_rhs(
        Emax, nu, basis_v, basis_s, λ, u, q,
        φ
    )
    k2 = hamilton_jacobi_RK4_rhs(
        Emax, nu, basis_v, basis_s, λ, u, q,
        φ - 0.5 * dt * k1
    )
    k3 = hamilton_jacobi_RK4_rhs(
        Emax, nu, basis_v, basis_s, λ, u, q,
        φ - 0.5 * dt * k2
    )
    k4 = hamilton_jacobi_RK4_rhs(
        Emax, nu, basis_v, basis_s, λ, u, q,
        φ - dt * k3
    )
...
def reinit_RK4_rhs(basis_s, φ0, φ, eps: float = 1e-4):
    ...
    φ_grad_norm = skfem.asm(
        linar_φ_grad_norm, basis_s, φ_interp=basis_s.interpolate(φ)
    )
    sign_φ0 = φ0 / np.sqrt(φ0**2 + eps**2)
...
if __name__ == '__main__':
    ...
    for it in range(max_iter):
        ...
        J, vol = compute_objectives(
            basis_s,
            φ,
            K, u, vf
        )
        λ -= β * vol   # Vol = 体積比 - vf
        # λ += β * np.clip(vol, -0.05, 0.05)
        ...
        q = -u.copy()
        # q_filtered = solve_helmholtz_filter(mesh, q)
        print(np.max(np.abs(u)), np.max(np.abs(q)))
        φ_updated = hamilton_jacobi_RK4(
            Emax, nu, basis_v, basis_s, λ, u, q,
            φ,
            dt
        )
        ...
        φ_updated = reinit_RK4(
            basis_s, φ0, φ_updated, dt
        )

        change = np.linalg.norm(φ - φ_old) / np.linalg.norm(φ_old)
